...
func _ready() -> void:
	...
	for idx in range(resolution * resolution):
		...
		var texture_position = material.get_shader_parameter("texture_position")
		if texture_position is Texture2DRD:
			texture_position.texture_rd_rid = texture
		node.material = material
		var material_override := material.duplicate() as ShaderMaterial
		material_override.set_shader_parameter("idx", idx)
...
func _compute(time: float, func_index: int):
	super(time, func_index)

#	var texture_pos = material.get_shader_parameter("texture_position")
#	if texture_pos is Texture2DRD:
#		var data := rd.texture_get_data(texture_pos.texture_rd_rid, 0).to_float32_array()
#		print("  Texture Data:", data.slice(0, 40))
